...
description: insert-หรือ-update ด้วย upsert และรวมการเขียนหลายครั้งเป็น round trip เดียวด้วย bulkWrite ทั้งแบบ ordered และ unordered
sidebar:
  order: 6
---
...
เพราะไม่มี Katherine อยู่ acknowledgement จึงรายงาน `upsertedId` — `_id` ของ document ที่ถูกสร้างขึ้น `matchedCount` เป็น `0` ที่เป็นสัญญาณบอกคุณว่าเกิดการ insert ขึ้นแทนที่จะเป็นการ update:
...
<Mermaid code={bulkDiagram} title="bulkWrite อัด operation หลายรายการเข้าไปใน round trip เดียวและคืนผลลัพธ์ที่รวมกัน" />
...
<Callout title="round trip น้อยลง องค์ประกอบเดิม">
upsert และ bulk write ไม่ได้เพิ่มภาษา query ใหม่ — แต่อัด insert, update และ delete ที่คุณรู้อยู่แล้วให้ถึง server ในรอบเดียว นั่นคือสิ่งที่ "การทำให้ CRUD เร็ว" หมายถึงเป็นส่วนใหญ่ในทางปฏิบัติ
</Callout>
...
export const quizBulk = [
  {
    q: 'รัน upsert แล้ว filter ไม่ match document ที่มีอยู่เลย จะเกิดอะไรขึ้น?',
    options: [
  ...
  },
  {
    ...
    options: [
      'ทำให้ทุก operation เป็น atomic โดยอัตโนมัติ',
      'ตรวจความถูกต้องของแต่ละ document เทียบกับ schema',
      'ส่ง operation หลายรายการไปยัง server ใน round trip เดียว',
      'retry operation ที่ล้มเหลวให้อัตโนมัติ',
    ],
    answer: 2,
  },
  {
    ...
    options: [
      'รัน operation ที่เหลือต่อไปอยู่ดี',
      'หยุดตรงจุดที่ล้มเหลว operation ที่ตามมาจึงไม่ได้รัน',
      'roll back ทุก operation ที่ทำไปก่อนหน้า',
      'เปลี่ยนชุดนั้นเป็น unordered ให้อัตโนมัติ',
    ],
    answer: 1,
  },
  {
