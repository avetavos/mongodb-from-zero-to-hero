...
<Mermaid code={productionOverview} title="sharded cluster คือ replica set หลายชุดที่อยู่หลัง router โดยแต่ละ shard มี high availability ในตัวเอง" />
...
2. **Replica sets** — primaries, secondaries, oplog ที่คอยทำให้ทุกตัวซิงค์กัน และ election อัตโนมัติที่เลือก primary ใหม่เมื่อตัวหนึ่งล้มเหลว
...
4. **Performance และ profiling** — การหา query ที่ช้าด้วย database profiler, การอ่าน `explain` ซ้ำ, และวิธีแก้ไขทั่วไปที่ทำให้ query กลับมาเร็วอีกครั้ง
...
- **คิดว่าต้อง shard ตั้งแต่วันแรกเพราะ "ข้อมูลจะเยอะในอนาคต"** — replica set เดี่ยวรองรับ traffic ได้นานกว่าที่คนส่วนใหญ่คิด และ sharding เพิ่ม operational complexity ที่ไม่คุ้มถ้ายังไม่มีหลักฐานว่าเครื่องเดียวตามไม่ทันจริง
...
> 💡 ตัวอย่างจากของจริง
>
> **ระบบค้นหาและ log ขนาดใหญ่ทั่วไป** — ใช้ MongoDB sharded cluster เก็บข้อมูลระดับหลายพันล้าน document กระจาย write และ storage ข้ามหลาย shard แทนที่จะพยายาม scale เครื่องเดียวขึ้นเรื่อย ๆ
>
...
export const quizOverview = [
  {
    q: 'ฟีเจอร์ใดของ MongoDB ที่ให้ high availability — รอดจากการล้มเหลวของ server เดียว?',
    options: [
      'Sharding',
      'replica set',
      'database profiler',
  ...
  },
  {
    ...
    options: [
      ' server ขนาดใหญ่มากเครื่องเดียวที่มีหน่วยความจำเพิ่ม',
