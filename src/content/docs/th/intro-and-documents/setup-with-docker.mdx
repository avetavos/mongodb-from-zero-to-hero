...
- **ปักหมุดเวอร์ชันหลัก** อย่าง `mongo:8` แทนที่จะเป็น `mongo:latest` เพื่อให้การอัปเดต image ไม่ทำให้คุณตกใจด้วยเวอร์ชัน server ที่ต่างออกไป
...
|---------|---------|------|
| รัน MongoDB ผ่าน Docker | ล้างและสร้างใหม่ได้เร็ว ไม่ทิ้งอะไรถาวรบนเครื่อง และ environment ใกล้เคียง production ที่มักรัน container เหมือนกันในทีม | ต้องมี Docker engine ทำงานอยู่ตลอด และมี overhead เล็กน้อยของ container เทียบกับการรันตรง ๆ บน host |
| ติดตั้ง MongoDB บนเครื่องโดยตรง | ไม่มี layer ของ container คั่นกลาง เชื่อมต่อผ่าน `localhost` ตรง ๆ | reset หรือปรับเวอร์ชันยากกว่า และแต่ละเครื่องในทีมอาจลง version หรือ config ไม่ตรงกัน |

## ข้อผิดพลาดที่พบบ่อย

- **ลืมปักหมุด image tag แล้วใช้ `mongo:latest`** — เวลา pull image ใหม่ เวอร์ชัน server อาจเปลี่ยนแบบไม่ตั้งใจและพัง config หรือ behavior ที่เคยพึ่งพา ควรปักหมุดเวอร์ชันหลักอย่าง `mongo:8` เสมอ
...
export const quizDocker = [
  {
    q: 'ทำไมต้อง mount named volume ไว้ที่ /data/db ตอนรัน container mongo?',
    options: [
      'ทำให้ MongoDB เริ่มทำงานเร็วขึ้น',
      'เก็บข้อมูลไว้นอก container เพื่อให้สร้าง container ใหม่แล้วข้อมูลไม่หาย',
      'จำเป็นต่อการที่ container จะเริ่มทำงานได้เลย',
      'ปิดการใช้งาน authentication',
    ],
    answer: 1,
  },
  {
    q: 'ค่า default แล้ว MongoDB ฟังอยู่ที่พอร์ตไหน?',
    options: ['8081', '5432', '27017', '3306'],
    answer: 2,
  },
  {
    q: 'ตัวแปร MONGO_INITDB_ROOT_USERNAME และ PASSWORD มีผลตอนไหน?',
    options: [
      'ทุกครั้งที่ container restart',
      'เฉพาะครั้งแรก เมื่อไดเรกทอรีข้อมูลว่างเปล่า',
      'เฉพาะเมื่อ mongo-express ทำงานอยู่ด้วย',
      'ไม่มีผลเลย เพราะ image ไม่สนใจค่าพวกนี้',
    ],
    answer: 1,
  },
]
