...
description: MongoDB เก็บ document เป็น BSON อย่างไร, BSON type ที่เจอบ่อย, และ field _id กับ ObjectId เข้ารหัสอะไรไว้บ้าง
sidebar:
  order: 5
---
...
|---------|---------|------|
| BSON (type เพิ่มเติมอย่าง Date, ObjectId, Decimal128, Binary) | เก็บค่าตรง type จริง query และ sort ได้ถูกต้องตาม type เช่นเรียงตามวันที่จริงหรือคำนวณเงินแม่นยำด้วย Decimal128 | ไม่ portable เท่า JSON ทันที ระบบภายนอกที่รับเฉพาะ JSON text ต้องแปลง type พวกนี้ก่อน ซึ่งอาจสูญเสียความแม่นยำถ้าทำไม่ถูกวิธี |
| JSON ธรรมดา | portable ข้ามระบบได้ทันที อ่านง่าย เครื่องมือรองรับทุกที่ | มี type พื้นฐานน้อย (string, number, boolean, null, array, object) เท่านั้น ต้องเข้ารหัส Date และเลขแม่นยำสูงเป็น string เอง แล้วเสี่ยงตีความผิดฝั่ง |
...
- **คิดว่า BSON กับ JSON เป็นสิ่งเดียวกัน** — คุณ*เขียน*และ*อ่าน*เป็น JSON ผ่าน shell/driver แต่สิ่งที่เก็บบนดิสก์คือ BSON การไม่รู้ความต่างนี้ทำให้งงว่าทำไม type ตัวเลขหรือวันที่ถึงหน้าตาเปลี่ยนไปตอนย้ายข้อมูลผ่านระบบที่ใช้ JSON text ล้วน
...
>
> **ระบบ metadata สเกลใหญ่ทั่วไป** — ใช้ ObjectId เป็นทั้ง primary key และตัวช่วยเรียงลำดับคร่าว ๆ ตามเวลาสร้างของ document จำนวนมหาศาล โดยไม่ต้องเก็บ field `created_at` แยกต่างหากสำหรับ use case ที่ไม่ต้องการความแม่นยำระดับมิลลิวินาที

export const quizBson = [
  {
    q: 'BSON ย่อมาจากอะไร และทำไม MongoDB ถึงใช้?',
    options: [
      'Basic Storage Object Notation; เรียบง่ายกว่า JSON',
      'Binary JSON; รูปแบบไบนารีที่กระชับซึ่งเพิ่ม type จริงอย่างวันที่และ ObjectId',
      'Buffered Serial Object Network; stream ข้อมูลได้เร็วกว่า',
      'Block Structured Object Notation; เก็บข้อมูลเป็นบล็อกขนาดตายตัว',
    ],
    answer: 1,
  },
  {
    q: 'ส่วนแรกของ ObjectId เข้ารหัสอะไรไว้?',
    options: [
      'ตัวเลขสุ่มเท่านั้น',
      'timestamp 4 ไบต์ของตอนที่สร้าง ObjectId',
      'ชื่อ collection',
      'ขนาดของ document',
    ],
    answer: 1,
  ...
  {
    q: 'ควรใช้ BSON type ไหนเก็บจำนวนเงินให้แม่นยำ?',
